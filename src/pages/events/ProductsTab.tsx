...
import { createEventProduct, listEventProducts, listProducts } from '../../lib/dataService'
          ...
          {items.map((item) => (
            <div key={item.id} className="flex flex-wrap items-center gap-3 bg-white border border-beetz-dark/5 rounded-xl p-4">
              <div className="flex-1 min-w-[140px]">
                ...
                <p className="text-xs text-beetz-dark/50">{item.quantity} un. × {currency(item.unit_price)} {item.notes ? `· ${item.notes}` : ''}</p>
              </div>
              <span className="font-bold text-sm">{currency(item.total)}</span>
            </div>
          ))}
          {items.length === 0 && <p className="text-sm text-beetz-dark/50">Nenhum registro corresponde aos seus critérios especificados.</p>}
        </div>
      )}
    </div>
  )
